...
def second():
	...
	#n=5	
	#attempt1=[attempt[i * n:(i + 1) * n] for i in range((len(attempt) + n - 1) // n )]
	#for i in range(0,len(attempt1)):
		#pie2(attempt1[i])
	
	
	#n=5	
	#outcome1=[outcome[i * n:(i + 1) * n] for i in range((len(outcome) + n - 1) // n )]
	#for i in range(0,len(outcome1)):
		#pie4(outcome1[i])
		#pie3(outcome1[i])
	
def totalScore(your_score,sno):	
	n=5	
	final = [your_score[i * n:(i + 1) * n] for i in range((len(your_score) + n - 1) // n )]	
	ele=[]
	total_score=[]
	for i in range(0,len(final)):
		total_score+=[sum(final[i])]

	return percentile(total_score,sno),total_score[sno]
...
def bar(time,i):
	...
	#my_file= +str(i)+'.png'
	plt.savefig('C:\Projects\ReportCard\\bar\\'+str(i)+'.png')
	plt.close()
...
def pie2(attempt):
	...
	#my_file = +str(i)+'.png'
	#plt.savefig(os.path.join(my_path,/+str(i)+'.png')
